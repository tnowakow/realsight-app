        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-950/50 border-b border-slate-800">
              <tr>
                <th onClick={() => handleSort('business_name')} className="group text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-slate-300">Tenant Name <SortIndicator columnKey="business_name" /></th>
                <th onClick={() => handleSort('property_id')} className="group text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-slate-300 hidden md:table-cell">Property <SortIndicator columnKey="property_id" /></th>
                <th onClick={() => handleSort('monthlyRent')} className="group text-right px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-slate-300">Monthly Rent <SortIndicator columnKey="monthlyRent" /></th>
                <th onClick={() => handleSort('amountOwed')} className="group text-right px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-slate-300">Amount Owed <SortIndicator columnKey="amountOwed" /></th>
                <th onClick={() => handleSort('paymentStatus')} className="group text-center px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-slate-300">Status <SortIndicator columnKey="paymentStatus" /></th>
                <th onClick={() => handleSort('daysPastDue')} className="group text-center px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-slate-300">Days Past Due <SortIndicator columnKey="daysPastDue" /></th>
                <th className="px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800">
              {sortedTenants.map((tenant) => (
                <tr key={tenant.id} className={`hover:bg-slate-800/50 transition-colors ${tenant.daysPastDue > 30 ? 'bg-red-500/5' : tenant.daysPastDue > 15 ? 'bg-amber-500/5' : ''}`}>
                  <td className="px-4 py-3"><div className="flex items-center gap-3"><div className={`w-2 h-2 rounded-full ${tenant.credit_rating?.startsWith('A') ? 'bg-emerald-500' : tenant.credit_rating?.startsWith('B') ? 'bg-blue-500' : 'bg-orange-500'}`} /><div><div className="text-sm font-medium text-white">{tenant.business_name}</div><div className="text-xs text-slate-500">{tenant.business_type}</div></div></div></td>
                  <td className="px-4 py-3 text-sm text-slate-400 hidden md:table-cell">{tenant.property_id === 'p1' ? 'Detroit Tech Center' : tenant.property_id === 'p2' ? 'Riverside Retail Plaza' : tenant.property_id === 'p3' ? 'Industrial Warehouse A' : 'Unknown'}</td>
                  <td className="px-4 py-3 text-right"><span className="text-sm font-semibold text-white">${tenant.monthlyRent.toLocaleString()}</span></td>
                  <td className="px-4 py-3 text-right"><span className={`text-sm font-bold ${tenant.amountOwed === 0 ? 'text-emerald-400' : tenant.amountOwed <= tenant.monthlyRent / 2 ? 'text-blue-400' : 'text-red-400'}`}>${tenant.amountOwed.toLocaleString()}</span></td>
                  ...
                  <td className="px-4 py-3"><button className="text-sm text-blue-400 hover:text-blue-300 font-medium transition-colors">View Details →</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        {sortedTenants.length === 0 && (<div className="text-center py-12"><DollarSign className="w-12 h-12 text-slate-600 mx-auto mb-3" /><p className="text-slate-400 font-medium">No tenants found</p><p className="text-sm text-slate-500 mt-1">Select a portfolio and property to view tenant financials</p></div>)}
      </div>
